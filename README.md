...
- **资源复用（v2）**：知识点、测试题、语音按单词维度复用，跨卡片组共享，减少 LLM 调用和存储冗余
- **随机测试**：基于复习计数的间隔巩固，答对减计数、答错重置，科学安排复习节奏
- **AI 对话助手**：基于 Spring AI + RAG 知识库的智能对话，支持普通聊天和 ReactAgent 智能体模式
- **文档管理**：在线 Markdown 编辑、批量导入、发布到向量库，独立的 AI 辅助写作入口
- **声音复刻**：集成阿里 CosyVoice，支持 TTS 音色管理和声音复刻
- **Web 后台管理**：用户管理、卡片组管理、知识库管理、文档管理、AI 对话日志、可观测性仪表盘
...
拍照 → OCR → 选词 → 创建卡片组
    → AI 生成 word_contents
    → 拆分为 knowledge_points（纯索引）
    → 分步学习（card_kp_progress 跟踪进度）
    → 测试（题库复用，4 种题型）
    → 全部通过 → 卡片组完成
    → 随机测试（review_count 计数，间隔巩固）
```
...
```
同一单词跨卡片组：
  knowledge_points  → 1 份索引，多卡片共享
  test_questions     → 1 份题库，按 word_id 复用
  word_audios        → 1 份音频，按 word_id + voice_id 缓存
  card_kp_progress   → 每卡片独立进度
```
