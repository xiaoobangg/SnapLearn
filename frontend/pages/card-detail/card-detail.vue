<template>
	<view class="card-detail">
		...
		<view class="status-banner done" v-if="learnStatus==='learn_done'" @click="goTest"><text>&#x2705; 学习完成，点击开始测验
				&#x27A1;</text></view>
		...
		<view class="relearn-banner" v-if="relearnCount>0 && !stepperFinished"><text>{{ relearnCount }} 个单词需再学</text>
		</view>
		<KnowledgeStepper v-if="learnStatus!=='test_done'" ref="stepperRef" :cards="cardsWithKps"
			:all-cards="cardsWithAllKps" card-id-key="id" completion-title="&#x1F389; 本组学习完成！"
			:completion-desc="relearnCount>0 ? `${relearnCount} 个单词需要再学一轮` : '所有单词已掌握，可以开始测验了'" @back="goBack"
			@relearn="onCardRelearn" @test="goTest" />
		...
		<view class="sheet-overlay" v-if="showMoveSheet" @click="showMoveSheet=false">
			<view class="sheet-panel" @click.stop>
				<view class="sheet-header"><text class="sheet-title">移动「{{ currentCard?.word }}」</text><text
						class="sheet-close" @click="showMoveSheet=false">&#x2715;</text></view>
				<view class="sheet-new"><input class="new-input" v-model="newGroupTitle"
						placeholder="&#x2795; 新建卡片组名称" /><button class="btn-new" @click="moveToNewGroup"
						:disabled="!newGroupTitle.trim()">&#x2795; 新建并移动</button></view>
				...
				<scroll-view scroll-y class="sheet-body" v-if="otherGroups.length>0">
					<view class="sheet-item" v-for="g in otherGroups" :key="g.id" @click="moveToGroup(g.id)">
						...
						<view class="si-body"><text>{{ g.title || '未命名卡片组' }}</text><text>{{ g.card_count || 0 }}
								词</text></view><text>&#x27A1;</text>
					</view>
				</scroll-view>
			</view>
		</view>
...
<script setup lang="ts">
	...
	const showMoveSheet = ref(false); const newGroupTitle = ref(""); const otherGroups = ref<any[]>([]); const relearnCardIds = reactive<Set<string>>(new Set());
	const displayProgress = computed(() => cardsWithKps.value.filter((c: any) => c.cardStatus !== "unlearned").length);
		const progressPercent = computed(() => cards.value.length > 0 ? Math.round(displayProgress.value / cards.value.length * 100) : 0);
	...
	const cardsWithKps = computed(() => {
		const mastered = stepperRef.value?.masteredCards ?? new Set<string>();
		const relearn = stepperRef.value?.relearnCards ?? new Set<string>();
		const list = cards.value.map(c => {
			let s = (c as any).cardStatus || (c as any).card_status || "unlearned";
			if (mastered.has(c.id)) s = "mastered";
			if (relearn.has(c.id)) s = "relearn";
...
<style lang="scss" scoped>
	...
	.sheet-divider {
		display: flex;
		align-items: center;
		padding: 8rpx $spacing-xl $spacing-base;
		color: $text-muted;
	}
