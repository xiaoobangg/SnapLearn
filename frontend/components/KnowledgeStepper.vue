...
<script setup lang="ts">
	...
	function speakWord(word : string) { const t = uni.getStorageSync("access_token"); const w = (currentCard.value as any)?.word_id || ""; uni.request({ url: `${getApiBaseUrl()}/tts?text=${encodeURIComponent(word)}&wordId=${w}&type=word`, header: { Authorization: `Bearer ${t}` }, success: (r : any) => { if (r.data?.audio_url) playTtsUrl(getServerBaseUrl() + "/" + r.data.audio_url); } }); }
	function speakKp(kp : KnowledgePointLike) { if (!kp.content) return; const t = uni.getStorageSync("access_token"); const w = (currentCard.value as any)?.word_id || ""; uni.request({ url: `${getApiBaseUrl()}/tts?text=${encodeURIComponent(kp.content)}&wordId=${w}&type=${kp.point_type || kp.pointType}`, header: { Authorization: `Bearer ${t}` }, success: (r : any) => { if (r.data?.audio_url) playTtsUrl(getServerBaseUrl() + "/" + r.data.audio_url); } }); }
