	...
	<view class="ks-root">
		<view class="done" v-if="finished">
			...
			<view class="bottom-bar">
				<view class="kp-btn outline" @click="$emit('back')">&#x1F3E0; 返回首页</view>
...
<script setup lang="ts">
	import { ref, computed, reactive, onMounted, getCurrentInstance } from "vue"; import { api } from "@/api"; import { getApiBaseUrl, getServerBaseUrl } from "@/config";
	...
	const props = withDefaults(defineProps<{ cards : CardLike[]; cardIdKey ?: string; completionTitle ?: string; completionDesc ?: string; allCards ?: CardLike[]; }>(), { cardIdKey: "id", completionTitle: "学习完成！", completionDesc: "", allCards: () => [] });
	const emit = defineEmits<{ back : []; mastered : [cardId: string]; relearn : [cardId: string]; test : []; }>();
	...
	async function markMastered(i : number) { const card = props.cards[i]; if (!card) return; try { await api.markCard(card.id, true); } catch (_e) { } masteredCards.add(cardId(card)); advanceCard(i); }
	async function markRelearn(i : number) { const card = props.cards[i]; if (!card) return; try { await api.markCard(card.id, false); } catch (_e) { } relearnCards.add(cardId(card)); emit("relearn", cardId(card)); advanceCard(i); }
...
<style lang="scss" scoped>
	...
	.bottom-bar {
		...
		right: 0;
		background: $bg-card;
		padding: $spacing-md $spacing-xl 40rpx;
		z-index: 100;
		box-shadow: 0 -4rpx 24rpx rgba(255, 138, 155, 0.08);
